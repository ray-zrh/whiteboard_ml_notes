...
# Fit the model
clf = svm.SVC(kernel='linear', C=1000) # Large C for hard margin
clf.fit(X, Y)
...
# Plot the lines
plt.plot(xx, yy, 'k-', label='Decision Boundary ($w^Tx + b = 0$)')
plt.plot(xx, yy_down, 'k--', label='Margin ($w^Tx + b = -1$)')
plt.plot(xx, yy_up, 'k--', label='Margin ($w^Tx + b = 1$)')
...
# Save the figure
output_path = os.path.join(output_dir, "ch06_hard_margin.png")
